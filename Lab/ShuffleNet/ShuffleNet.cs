...
public class ShuffleNet
{
    ...
    public ShuffleNet()
    {
        session = new InferenceSession("C:\\Users\\dimav\\OneDrive\\Рабочий стол\\Programming\\Lab\\ShuffleNet\\shufflenet-v2-10.onnx"); ;
    }
    public async Task<List<(String, float)>> proccessImage(String filename, CancellationToken token)
    {
        return await Task<List<(string, float)>>.Factory.StartNew(() =>
        {
            ...
            //используем семафор, так как нейросеть может быть запущена в нескольких потоках
            sem.WaitOne();
            ...
            // Возвращаем 10 наиболее вероятных результатов
                ...
                .Take(10))
            {
                ...
                res.Add((p.Label, p.Confidence));
                Console.WriteLine(p.Label + p.Confidence);
    ...
    //ОбЪекты, которые нейросеть может распознать
